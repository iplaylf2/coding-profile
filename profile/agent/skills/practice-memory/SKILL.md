...
- What future behavior is the user trying to improve: activation, planning, context gathering, editing discipline, wording judgment, validation, communication, handoff, or a domain-specific workflow?
...
- Is this a skill gap because the agent skipped a predictable context source, asked avoidable questions, failed validation, confused routing with execution, gave phrasing, examples, or local convention more weight than the task required, or produced output the skill should have prevented?
...
- Should an existing skill change because the desired future behavior belongs to that skill's task family and the activation boundary is already right?
- Should a new skill exist because the lesson names a recurring task family with its own activation boundary, information sources, decisions, and validation responsibilities?
- What context should remain only as session background because it explains this session rather than future behavior?
...
- Did the skill activate correctly but guide the wrong work, miss relevant context, ask unnecessary questions, lose the user's center of gravity, give prompt phrasing more weight than task facts, or report poorly? If so, revise the body.
...
- Use the session record to classify the issue, but separate portable behavior from session-specific detail before editing durable guidance.
