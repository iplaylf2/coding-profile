...
- What future behavior is the user trying to improve: activation, planning, evidence gathering, editing discipline, validation, communication, handoff, or a domain-specific workflow?
...
- Did the agent eventually converge by learning a stable rule, or only by following one-off instructions from the user?
...
- Is this a skill gap because the agent skipped a predictable context source, asked avoidable questions, failed to validate a risky change, mixed routing guidance with execution workflow, or produced output the skill should have prevented?
...
- Is this no durable change because the issue was a one-off bug, transient misunderstanding, private fact, preference that only applied once, or constraint that should not be generalized?
...
- Did the skill activate correctly but guide the wrong work, miss evidence, ask unnecessary questions, lose the user's center of gravity, or report the result poorly? If so, revise the body.
...
- Use session evidence as the source of truth; do not turn every inconvenience into policy.
